...
//    this.iCellWidth = this.$node.width() / this.iWidth;
  ...
  Board.prototype = {
    ...
    /**
     * Dynamically generate a map provided by the options.
     * @param oOpts {object} Optional. Map generation options.
     */
    generateMap: function (oOpts) {
      ...
      for (i = 0; i < this.iXLen; i += 1) {
        aRow = [];

        for (j = 0; j < this.iYLen; j += 1) {
          aRow[j] = (bApplyBorder && (
            0 === i || j === 0 || i === this.iXLen - 1 ||
            j === this.iYLen - 1)) ? '*' : ' ';
        }
     ...
     * @param aExcludedPos {array} Optional. The first extra position to
     * also exclude.
     * @param aExcludedPosN {array} Optional. Any number of extra positions to
     * also exclude.
     * @return {array} A position.
     */
    getRandomEmptyPosition: function(aExcludedPos, aExcludedPosN) {
      var iX;
      var iY;

      // todo: implement excluded positions
